...
2. 문자함수 : lower, upper, length, substr, ltrim, rtrim, trim -- 소문자, 대문자, 길이, trim:공백제거
...
5. 그룹(집합)함수(다중행 함수) : avg, sum, max, min, count
...
--데이터 베이스 테이블 생성할 때는 한번만 실행--컬럼  자료형(기억공간의 크기)					
insert into text(str1,str2) values('엘리트','엘리트'); 
--insert into : 데이터 추가 명령  저장하고자하는 컬럼  
--values: 해당하는 값 저장
insert into text(str1,str2) values('elite','elite');
commit;
-- 트랜잭션의 마무리 저장
--트랜잭션: 일련의 작업단위 롤백: 수행한거 실행취소같은거(데이터 베이스 안에서 처리) 각각 처리 과정에서 문제가 없다면, 커밋 처리. 
--단, 커밋처리하는 중에도 익셉션이 발생할 수 있다. 이때도 롤백할 수 있음.
select * from tab;
select * from text;
...
select instr('HelloToWorld','o',-5) from dual; -- -5는 시작위치

--ex9) 지정한 길이의 문자열을 추출 : substr(표현식,시작,[갯수])  
select substr('I am very happy', 6, 4) from dual;--여섯번째 자리에서 4글자
select substr('I am very happy', 6) from dual; -- 여섯번째 자리에 끝까지 출력
...
select to_char(to_date('15/1/5'),'YYYY"년" MM"월" DD"일"') as "오늘날짜" from dual; --날짜 서식을 문자형태로 바꿔서 출력
select to_char(to_date('15/01/05'),'YYYY"년" fmMM"월" DD"일"') from dual; --fm(format): 유효하지 않은 숫자 제거/추가
